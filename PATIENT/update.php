...
$uploaddir = 'img/';//folder for upload
	...
	else
	{
		...
		$sql= "UPDATE patient SET pname='$pname',fname='$fname',email='$email',month='$month',day='$day',year='$year',gender='$gender',mblno='$mblno',disease='$disease',photo='$photo' where pid='$pid'";
		if ($conn->query($sql) === TRUE) 
		{
			//there is probleom in usin word 'add' to overcome the bellow line is used
			$conn->query("UPDATE `hm`.`patient` SET `add` = '$add' WHERE `patient`.`pid` = 2;");
